...
int IntroducerAsyncSelfIntroduction(const std::shared_ptr<::trpc::test::issueimpl::IntroducerServiceProxy>& proxy) {
  ...
  bool succ = true;
  ::trpc::Latch latch(1);
  proxy->AsyncSelfIntroduction(client_ctx, req).
      Then([&latch, &succ](::trpc::Future<::trpc::test::issueimpl::IntroduceReply>&& fut) {
        if (fut.IsReady()) {
...
int Run() {
  ...
  return IntroducerAsyncSelfIntroduction(proxy);
}
